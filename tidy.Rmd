...
library(statmod)
library(AICcmodavg)
library(MASS) #need to call dplyr::select specifically while MASS is loaded
```
...
veg_attributes <- veg_attributes %>% 
  mutate(gps = name) %>% dplyr::select(OBJECTID, CID, gps, POINT_X, POINT_Y, Z)

island_veg_all <- island_veg %>% 
  left_join(veg_attributes, island_veg, by = "gps") %>%
  slice(-13,-14,-15,-17,-18,-19,-21,-22,-23,-25,-26,-27,-29,-30,-31,-33,-34,-35,-37,-38,-39,-41,-42,-43) %>%
  dplyr::select(location, date, gps, cover, height, spp, spp_code, POINT_X, POINT_Y, Z)
...
```{r recode-veg-height}
 island_veg_all <- island_veg_all %>%
mutate(height_class = case_when(TRUE ~ as.ordered(height))) %>% #adding a height class factor variable
mutate(cover_class = case_when(TRUE ~ as.ordered(cover))) #adding a cover class factor variable
```

```{r create veg type columns}
island_veg_all$type <- "NA"
island_veg_all$arm<- "NA"
```

```{r spp_list dataframe}
...
#spp_list
```{r}
spp_list <- island_veg_all %>% dplyr::select(spp, type, arm)
...
filtered <- island_veg_all %>%
  dplyr::select(location,gps,date, month, cover, cover_class, height, height_class, POINT_X, POINT_Y, Z, arm)
...
structure <- island_veg_all %>%
  ...
  mutate(gram = case_when(gram == "NULL" ~ "0",
                          TRUE ~ "1")) %>%
  dplyr::select(gps, herb, vine, gram, shrub)
...
# herg_ct <- read_excel("gitdata/herg_buff_count_TableToExcel.xlsx")
# coei_ct <- read_excel("gitdata/coei_buff_count_TableToExcel.xlsx")
# gbbg_ct <- read_excel("gitdata/gbbg_buff_count_TableToExcel.xlsx")

herg_ct <- read_excel("gitdata/plot_buff_5m_herg_TableToExcel.xlsx")
coei_ct <- read_excel("gitdata/plot_buff_5m_coei_TableToExcel.xlsx")
gbbg_ct <- read_excel("gitdata/plot_buff_5m_gbbg_TableToExcel.xlsx")
```

```{r read in GDI bird counts}
# gdi_herg_ct <- read_excel("gitdata/gdi_herg_buff_count.xls")
# gdi_coei_ct <- read_excel("gitdata/gdi_coei_buff_count.xls")
# gdi_gbbg_ct <- read_excel("gitdata/gdi_gbbg_buff_count_TableToExcel.xlsx")

gdi_herg_ct <- read_excel("gitdata/gdi_buff_5m_herg_TableToExcel.xlsx")
gdi_coei_ct <- read_excel("gitdata/gdi_buff_5m_coei_TableToExcel.xlsx")
gdi_gbbg_ct <- read_excel("gitdata/gdi_buff_5m_gbbg_TableToExcel.xlsx")
```

```{r fix GDI bird counts}
gdi_herg_ct <- gdi_herg_ct %>%
  mutate(gps = paste0('gdi',gdi_herg_ct$OBJECTID))

gdi_coei_ct <- gdi_coei_ct %>%
  mutate(gps = paste0('gdi',gdi_coei_ct$OBJECTID))

gdi_gbbg_ct <- gdi_gbbg_ct %>%
  mutate(gps = paste0('gdi',gdi_gbbg_ct$OBJECTID))
```

```{r tidy and join bird dataframes (make _buffs & .vars) }

#combine GDI and all other islands
herg_buff <- bind_rows(gdi_herg_ct, herg_ct)
coei_buff <- bind_rows(gdi_coei_ct, coei_ct)
gbbg_buff <- bind_rows(gdi_gbbg_ct, gbbg_ct)

#trim down to sampled points
herg_buff <- herg_buff %>%
  dplyr::select(Join_Count, gps, species) %>%
...
```
#get veg.var dataframes for models
```{r veg.vars}
n_herg <- herg_buff %>%
...
richness <- model.vars %>%
  dplyr::select(gps, sp_richness)

veg.vars.herg <- left_join(island_veg_all, n_herg, by = "gps")
veg.vars.gbbg <- left_join(island_veg_all, n_gbbg, by = "gps")
veg.vars.coei <- left_join(island_veg_all, n_coei, by = "gps")

veg.vars.v <- island_veg_all %>%
  left_join(n_herg, by = "gps") %>%
  left_join(n_gbbg, by = "gps") %>%
  left_join(n_coei, by = "gps") %>%
  left_join(richness, by = "gps") %>%
  dplyr::select(location, month, gps, Z, spp, spp_code, genus, type, sp_richness, cover, height, herg_count, gbbg_count, coei_count) #can decide later if you need POINT_X and POINT_Y or classed height/cover, etc

veg.vars.v <- veg.vars.v %>%
  mutate(bird_count = herg_count + gbbg_count + coei_count) %>% 
  mutate(spp = str_replace_all(spp,' ', '_'),
         spp_code = str_replace_all(spp_code, ' ', '_'))

#make version without species and genus (no duplicate GPS ids) to test how lm is treating them
  ##duplicate entries was screwing up the model *sigh* -> need to pivot veg.vars.v for species & genus models
veg.vars <- veg.vars.v %>% 
  dplyr::select(location, month, gps, Z, type, sp_richness, cover, height, herg_count, gbbg_count, coei_count, bird_count) %>% 
  distinct(gps, .keep_all = T)

#species testable df
spp.vars <- veg.vars.v %>% 
  dplyr::select(month, location, gps, Z, sp_richness, herg_count, gbbg_count, coei_count, bird_count, spp,) %>%
  pivot_wider(names_from = spp, names_sort = T, values_from = spp, values_fn = list, values_fill = as.list("0"))
