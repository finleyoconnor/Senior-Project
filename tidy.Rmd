...
```{r recode-veg-height}
 island_veg_all <- island_veg_all %>%
mutate(height_class = case_when(TRUE ~ as.factor(height))) %>% #making height class a factor variable
mutate(cover_class = case_when(TRUE ~ as.factor(cover))) #making cover class a factor variable
```
...
filtered <- island_veg_all %>%
  select(location,gps,date, cover, cover_class, height, height_class, POINT_X, POINT_Y, Z, arm)
