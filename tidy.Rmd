...
island_veg_all <- island_veg %>% 
  ...
  dplyr::select(location, date, gps, cover, height, spp, POINT_X, POINT_Y, Z)
...
veg.vars.v <- island_veg_all %>%
  ...
  dplyr::select(location, month, gps, Z, spp, genus, type, sp_richness, cover, height, herg_count, gbbg_count, coei_count) #can decide later if you need POINT_X and POINT_Y or classed height/cover, etc
...
#make version without species and genus (no duplicate GPS ids) to test how lm is treating them
  ##duplicate entries was screwing up the model *sigh* -> need to pivot veg.vars.v
veg.vars <- veg.vars.v %>% 
