...
##veg models
```{r lm veg}
lm.sp_richness <- glmer(sp_richness ~  + height + (1|location), family = poisson(link = log),data = veg.vars)
...
```{r richness ~ nest counts}
lm.richbirds <- glmer(sp_richness ~
                      herg_count +
                      gbbg_count +
                      coei_count +
                      (1|location/gps),
                    ...
                    data = veg.vars)
plot(lm.richbirds)
isSingular(lm.richbirds)
qqnorm(resid(lm.richbirds))
vif(lm.richbirds)
summary(lm.richbirds)

lm.richbirds1 <- glmer(sp_richness ~
                      herg_count +
                      gbbg_count +
                      coei_count +
                      (1|month),
                    ...
                    data = veg.vars)
plot(lm.richbirds1)
isSingular(lm.richbirds1)
qqnorm(resid(lm.richbirds1))
vif(lm.richbirds1)
summary(lm.richbirds1)
```
```{r cover ~ nest cts}
lm.cover <- glmer(cover ~
                      herg_count +
                      gbbg_count +
                      coei_count +
                      (1|location), #/gps caused singularity
                    family = poisson(link = log),
                    data = veg.vars)
plot(lm.cover)
isSingular(lm.cover)
qqnorm(resid(lm.cover))
vif(lm.cover)
summary(lm.cover)

lm.cover1 <- glmer(cover ~
                      herg_count +
                      gbbg_count +
                      coei_count +
                      (1|month),
                    ...
                    data = veg.vars)
plot(lm.cover1)
isSingular(lm.cover1)
qqnorm(resid(lm.cover1))
vif(lm.cover1)
summary(lm.cover1)
```
```{r solidago ~ nest ct}
