...
# lm.coei <- lme4::glmer(Join_Count ~ Z + height + cover + sp_richness + herb + vine + gram + shrub + (1|location), family = poisson (link = log), data = coei.vars.2)
# 
# 
# lm.coei
# plot(lm.coei)
...
lm.gbbg1 <- lme4::glmer(Join_Count~ 
                           Z +
                           cover +
                           height +
                           sp_richness + 
  ...
  plot(lm.gbbg1)
  summary(lm.gbbg1)
  qqnorm(resid(lm.gbbg1))

lm.gbbg2 <- lme4::glmer(Join_Count ~
                          herb *
                          #vine *
                          gram *
                          shrub +
...
lm.gull <- lme4::glmer(Join_Count ~ Z + height + cover + sp_richness + herb + vine + gram + shrub + (1|location), family = poisson (link = log), data = gull.vars.2)
...
cor.test(x = herg.vars.2$cover, y = herg.vars.2$height, method = 'spearman')
```
```{r gbbg cor}
vif(lm.gbbg1)
```
